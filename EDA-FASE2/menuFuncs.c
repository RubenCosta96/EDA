...
void managerMenu(Manager **head, Vehicle **vehicle, Client **client, Manager *m, Graph **g, History **hist)
{
  ...
  case 10:
    ...
    break;
  case 11:
    // Find shortest route with given vertexes;
    fflush(stdout);
    clearConsole();
    ...
    printf("You want to go from what location to what location?\n");
    listVertexes(g);
    printf("Start: ");
    scanf("%d", &initial);
    printf("\n");
    ...
    convertIdToLocation(&gAux, initial, location);
    char initialLocation[SIZE];
    strcpy(initialLocation, location);

    convertIdToLocation(&gAux, final, location);
    char finalLocation[SIZE];
    strcpy(finalLocation, location);

    dijkstra(gAux, initialLocation, finalLocation, dist, 0);
    enterToContinue();
    break;
  case 12:
    clearConsole();
    printf("All available vertexes:\n");
    listVertexes(g);
    enterToContinue();
  case 0:
    clearConsole();
