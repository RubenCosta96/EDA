...
{
	Vehicle *head = *v;
	printf("ID\tType\t\tAutonomy\tMax Autonomy\tBattery\tCost\tRented By\tLocation\n");
	for (; head != NULL; head = head->next)
	{
		printf("%d\t%s\t%d\t%d\t%.2f\t%.2f\t%d\t\t%s\n", head->id, head->type, head->autonomy, head->maxAutonomy, head->battery, head->cost, head->rentedBy, head->location);
	}
}
 ...
 * @param vehicleID
 */
float cancelRental(Vehicle **head, Graph **graph, int vehicleID)
{
	Graph *aux = *graph;
	Vehicle *currentV = *head;
	...
	else
	{
		...
		float dist[SIZE];
		float lowestDist = dijkstra(aux, currentV->location, finalVertex, dist, 1);
		currentV->autonomy = currentV->autonomy - (((float)100.00 * lowestDist) / currentV->maxAutonomy);
		clearConsole();
		printf("New autonomy: %d\n", currentV->autonomy);

		currentV->rentedBy = 0;

		return lowestDist * currentV->cost;
	}
}
...
void vehicleInGeocode(Vehicle **v, Graph **g, int locId)
{
	Vehicle *vAux = *v;
	Graph *gAux = *g;
	...
	convertIdToLocation(g, locId, auxVertex);

	while (vAux != NULL)
	{
		if (strcmp(auxVertex, vAux->location) == 0)
		{
