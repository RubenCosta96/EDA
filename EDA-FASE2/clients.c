...
void clientMenu(Client **head, Vehicle **vehicle, Client *c, Manager **manager, Graph **g, History **hist)
{
	...
	case 3:
		...
		else
		{
			...
			listVertexes(g);
			scanf("%d", &locationID);
			c->balance -= cancelRental(vehicle, g, vehicleID);
	...
	case 7:
		float distance;
		char location[SIZE], clientLoc[SIZE];
		Graph *gAux = *g;
		Vehicle *vAux = *vehicle;
		clearConsole();
		int spot;
		printf("Where are you at the moment? (Answer with the Location ID)\n");
		listVertexes(g);
		scanf("%d", &spot);
		printf("Within which distance you want to find a vehicle: ");
		scanf("%f", &distance);
		convertIdToLocation(&gAux, spot, location);
		strcpy(clientLoc, location);
		locationsBelowDistance(gAux, vAux, location, distance);
		break;
	case 8:
		char vertex[SIZE], newType[SIZE];
		int locId;
		clearConsole();
		printf("What location do you want to find vehicles in? (Insert location ID)\n");
		listVertexes(g);
		printf("->");
		getchar();
		scanf("%d", &locId);
		vehicleInGeocode(vehicle, g, locId);
		break;
	case 0:
		loginOrReg(head, manager, vehicle, g, hist);
		return;
